...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20240012_org_members_add_lark_app"
    }
}
        ...
        manager
            .alter_table(
                Table::alter()
                    ...
                    .add_column_if_not_exists(ColumnDef::new(OrgMembers::LarkAppId).string_len(64))
                    .to_owned(),
            )
            .await?;
        manager
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .alter_table(
                Table::alter()
                    .table(OrgMembers::Table)
                    .drop_column(OrgMembers::LarkAppId)
                    .to_owned(),
            )
            .await?;
        manager
            .alter_table(
                Table::alter()
                    .table(OrgMembers::Table)
                    .drop_column(OrgMembers::LarkAppSecret)
                    .to_owned(),
            )
            .await
    }
